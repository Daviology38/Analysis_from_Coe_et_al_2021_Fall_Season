...
for y in year:
    for m, d in zip(month, day_tot):
        for i in range(d):
            ...
            precip_total[count, :, :] = xr.open_dataset(
                "H:/persiann-cdr/files/PERSIANN-CDR_v01r01_" + str(y) + m + d + ".nc"
            ).precipitation.values[:, 959:1081, 39:121]
            lats_precip = xr.open_dataset(
                "H:/persiann-cdr/files/PERSIANN-CDR_v01r01_" + str(y) + m + d + ".nc"
            ).precipitation.lat.values[39:121]
            lons_precip = xr.open_dataset(
                "H:/persiann-cdr/files/PERSIANN-CDR_v01r01_" + str(y) + m + d + ".nc"
            ).precipitation.lon.values[959:1081]
            count = count + 1
