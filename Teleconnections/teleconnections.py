...
ao = pd.read_csv(
    "C:/Users/CoeFamily/Documents/David College Class Work/WT_analysis/Teleconnections/ao.csv"
)
nao = pd.read_csv(
    "C:/Users/CoeFamily/Documents/David College Class Work/WT_analysis/Teleconnections/nao.csv"
)
pna = pd.read_csv(
    "C:/Users/CoeFamily/Documents/David College Class Work/WT_analysis/Teleconnections/pna.csv"
)
nino = pd.read_csv(
    "C:/Users/CoeFamily/Documents/David College Class Work/WT_analysis/Teleconnections/nino.csv"
)
mjo = pd.read_csv(
    "C:/Users/CoeFamily/Documents/David College Class Work/WT_analysis/Teleconnections/mjo_mam.csv"
)
...
ao = ao.loc[
    (ao["Year"] >= 1979)
    & (ao["Year"] <= 2018)
    & ((ao["M\t"] == 3) | (ao["M\t"] == 4) | (ao["M\t"] == 5))
]
nao = nao.loc[
    (nao["Year"] >= 1979)
    & (nao["Year"] <= 2018)
    & ((nao["M"] == 3) | (nao["M"] == 4) | (nao["M"] == 5))
]
pna = pna.loc[
    (pna["Year"] >= 1979)
    & (pna["Year"] <= 2018)
    & ((pna["M"] == 3) | (pna["M"] == 4) | (pna["M"] == 5))
]
...
conds = [
    overall_data.values <= -1.0,
    overall_data.values >= 1.0,
    np.isnan(overall_data.values),
]
choices = ["Negative", "Positive", np.nan]

dff = pd.DataFrame(
    np.select(conds, choices, default="Neutral"),
    index=overall_data.index,
...
for i in range(1, 8, 1):
    ...
    NAO_actual[0, i - 1] = (
        dff.groupby(dff.WT).get_group(i).NAO.value_counts()["Negative"]
    )
    NAO_actual[1, i - 1] = (
        dff.groupby(dff.WT).get_group(i).NAO.value_counts()["Neutral"]
    )
    NAO_actual[2, i - 1] = (
        dff.groupby(dff.WT).get_group(i).NAO.value_counts()["Positive"]
    )

    PNA_actual[0, i - 1] = (
        dff.groupby(dff.WT).get_group(i).PNA.value_counts()["Negative"]
    )
    PNA_actual[1, i - 1] = (
        dff.groupby(dff.WT).get_group(i).PNA.value_counts()["Neutral"]
    )
    PNA_actual[2, i - 1] = (
        dff.groupby(dff.WT).get_group(i).PNA.value_counts()["Positive"]
    )

    NINO_actual[0, i - 1] = (
        dff.groupby(dff.WT).get_group(i).NINO.value_counts()["Negative"]
    )
    NINO_actual[1, i - 1] = (
        dff.groupby(dff.WT).get_group(i).NINO.value_counts()["Neutral"]
    )
    NINO_actual[2, i - 1] = (
        dff.groupby(dff.WT).get_group(i).NINO.value_counts()["Positive"]
    )
...
for j in range(1000):
    ...
    for i in range(1, 8, 1):
        AO_actualtemp[j, 0, i - 1] = (
            dff.groupby(dff.WT).get_group(i).AO.value_counts()["Negative"]
        )
        AO_actualtemp[j, 1, i - 1] = (
            dff.groupby(dff.WT).get_group(i).AO.value_counts()["Neutral"]
        )
        AO_actualtemp[j, 2, i - 1] = (
            dff.groupby(dff.WT).get_group(i).AO.value_counts()["Positive"]
        )

        NAO_actualtemp[j, 0, i - 1] = (
            dff.groupby(dff.WT).get_group(i).NAO.value_counts()["Negative"]
        )
        NAO_actualtemp[j, 1, i - 1] = (
            dff.groupby(dff.WT).get_group(i).NAO.value_counts()["Neutral"]
        )
        NAO_actualtemp[j, 2, i - 1] = (
            dff.groupby(dff.WT).get_group(i).NAO.value_counts()["Positive"]
        )

        PNA_actualtemp[j, 0, i - 1] = (
            dff.groupby(dff.WT).get_group(i).PNA.value_counts()["Negative"]
        )
        PNA_actualtemp[j, 1, i - 1] = (
            dff.groupby(dff.WT).get_group(i).PNA.value_counts()["Neutral"]
        )
        PNA_actualtemp[j, 2, i - 1] = (
            dff.groupby(dff.WT).get_group(i).PNA.value_counts()["Positive"]
        )

        NINO_actualtemp[j, 0, i - 1] = (
            dff.groupby(dff.WT).get_group(i).NINO.value_counts()["Negative"]
        )
        NINO_actualtemp[j, 1, i - 1] = (
            dff.groupby(dff.WT).get_group(i).NINO.value_counts()["Neutral"]
        )
        NINO_actualtemp[j, 2, i - 1] = (
            dff.groupby(dff.WT).get_group(i).NINO.value_counts()["Positive"]
        )

        MJO_actualtemp[j, 0, i - 1] = (
            dff.groupby(dff.WT).get_group(i).MJO.value_counts()["1"]
        )
        MJO_actualtemp[j, 1, i - 1] = (
            dff.groupby(dff.WT).get_group(i).MJO.value_counts()["2"]
        )
        MJO_actualtemp[j, 2, i - 1] = (
            dff.groupby(dff.WT).get_group(i).MJO.value_counts()["3"]
        )
        MJO_actualtemp[j, 3, i - 1] = (
            dff.groupby(dff.WT).get_group(i).MJO.value_counts()["4"]
        )
        MJO_actualtemp[j, 4, i - 1] = (
            dff.groupby(dff.WT).get_group(i).MJO.value_counts()["5"]
        )
        MJO_actualtemp[j, 5, i - 1] = (
            dff.groupby(dff.WT).get_group(i).MJO.value_counts()["6"]
        )
        MJO_actualtemp[j, 6, i - 1] = (
            dff.groupby(dff.WT).get_group(i).MJO.value_counts()["7"]
        )
        MJO_actualtemp[j, 7, i - 1] = (
            dff.groupby(dff.WT).get_group(i).MJO.value_counts()["8"]
        )
...
rr3 = ax1.bar(
    r3, AO_actual[2, :], color=color3, width=barWidth, edgecolor="white", label="+"
)
...
rr3 = ax2.bar(
    r3, NAO_actual[2, :], color=color3, width=barWidth, edgecolor="white", label="+"
)
...
rr3 = ax3.bar(
    r3, PNA_actual[2, :], color=color3, width=barWidth, edgecolor="white", label="+"
)
...
rr3 = ax4.bar(
    r3, NINO_actual[2, :], color=color3, width=barWidth, edgecolor="white", label="+"
)
